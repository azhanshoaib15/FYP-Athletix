...
# Key joints that matter for exercise analysis
# Ignores face/finger/ear landmarks which are often hidden or irrelevant
_KEY_JOINT_IDX = [11, 12, 13, 14, 15, 16, 23, 24, 25, 26, 27, 28]
# shoulders, elbows, wrists, hips, knees, ankles

def filter_low_confidence_frames(seq: np.ndarray, min_visibility: float = 0.15) -> np.ndarray:
    """
    Drop frames where key exercise joints have low visibility.
    Uses key joints only (shoulders/elbows/wrists/hips/knees/ankles)
    instead of all 33 landmarks — face and finger landmarks are
    often hidden but irrelevant to exercise form analysis.
    Keeps at least 5 frames as fallback even if all are low-confidence.
    """
    vis_all = seq.reshape(len(seq), 33, 4)[:, :, 3]        # (frames, 33)
    vis_key = vis_all[:, _KEY_JOINT_IDX].mean(axis=1)       # (frames,) key joints only
    mask    = vis_key >= min_visibility
    filtered = seq[mask]
