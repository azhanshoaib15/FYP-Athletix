...
import {
    ActivityIndicator, Alert, Dimensions, RefreshControl, ScrollView, StyleSheet,
...
export default function ProgressScreen({ onNavigate }: { onNavigate: (screen: any) => void }) {
    ...
    // Refresh data on mount to get latest profile + workout logs
    useEffect(() => {
        refreshSilent();
        // Second refresh after short delay to catch any stale cache
        const t = setTimeout(() => refreshSilent(), 1500);
        ...
        try {
            ...
            if (res.ok) {
                ...
                Alert.alert('Saved!', 'Progress logged successfully.', [{
                    text: 'OK',
                    onPress: () => refreshSilent(), // Second refresh after dismiss
                }]);
            } else {
    ...
    const buildWeeklyData = () => {
        ...
        const now     = new Date();
        // Count from workout sessions — try multiple date fields
        sessions.forEach((s: any) => {
            const dateStr = s.started_at || s.created_at || s.session_date || s.date;
            if (!dateStr) return;
            ...
            if (diff >= 0 && diff < 7) {
                ...
                counts[idx] = Math.min(counts[idx] + 1, 99); // cap to avoid chart overflow
            }
        });
    ...
    const hasWorkouts = useMemo(() => {
        // Show chart if ANY progress records exist (even weight-only logs)
        // so user always sees the chart after first interaction
        ...
        <ScrollView 
            ...
            <View style={st.card}>
                ...
                <View style={st.macrosRow}>
                    ...
                    <VictoryPie
                        ...
                        innerRadius={45}
                        colorScale={['#E24B4A','#EF9F27','#1D9E75']}
                        labels={[]}
            ...
            <View style={st.card}>
                <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center', marginBottom:4}}>
                    ...
                    <Text style={{color:'#00CC66', fontSize:10}}>● Live</Text>
                </View>
                {[
            ...
            {/* Recent workouts — shows sessions first, falls back to progress records */}
            <View style={st.card}>
                <Text style={st.cardTitle}>Recent Workouts</Text>
                {sessions.length === 0 && progress.filter((p:any) => (p.workouts_completed||0) > 0).length === 0 ? (
                    <View>
                        <Text style={st.emptyTxt}>No workouts logged yet.</Text>
                        <Text style={st.emptySubTxt}>Complete exercises in Workout Schedule and tap Log to see them here.</Text>
                    </View>
                ) : sessions.length > 0 ? (
                    ...
                    // Fallback: show progress records as workouts when no sessions exist
                    progress.filter((p:any) => (p.workouts_completed||0) > 0).slice(0,5).map((p: any, i: number) => (
                        <View key={i} style={st.sessionRow}>
                            <View>
                                <Text style={st.sessionDate}>
                                    {new Date(p.recorded_at).toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'})}
                                </Text>
                                <Text style={st.sessionTime}>
                                    {new Date(p.recorded_at).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}
                                </Text>
                            </View>
                            <View style={st.sessionRight}>
                                <Text style={st.sessionStat}>
                                    {p.total_workout_minutes ? p.total_workout_minutes + ' min' : 'Completed'}
                                </Text>
                                {p.total_calories_burned > 0 && (
                                    <Text style={st.sessionCal}>{Math.round(p.total_calories_burned)} kcal</Text>
                                )}
                            </View>
                        </View>
                    ))
...
const st = StyleSheet.create({
    ...
    backBtn:       { marginTop:50, marginBottom:16, padding:10, backgroundColor:'#1D9E75', borderRadius:20, alignSelf:'flex-start' },
