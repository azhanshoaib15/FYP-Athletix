...
const ML_URL      = 'https://humorous-optimism-production-7510.up.railway.app';
const BACKEND_URL = 'fyp-athletix-production.up.railway.app';
...
export default function FormAnalysisScreen({onNavigate}: Props) {
    ...
    const recTimerIvl = useRef<any>(null);
    ...
    useEffect(() => () => {
        ...
        clearInterval(recTimerIvl.current);
    }, []);
    ...
    const startRecording = () => {
        ...
        // Start recording timer — minimum 8 seconds before stop enabled
        setRecordingSeconds(0);
        let sec = 0;
        recTimerIvl.current = setInterval(() => {
            sec++;
            setRecordingSeconds(sec);
        }, 1000);

        // 400ms interval for faster frame accumulation and smoother skeleton
        ivl.current = setInterval(async () => {
            ...
            try {
                ...
                const res = await fetch(ML_URL + '/live/check', {
                    ...
                    body: JSON.stringify({
                        ...
                        recent_frames_b64: framesR.current.slice(-15),
                        total_reps_so_far: 0,
                        accumulated_keypoints: kpR.current.length > 0 ? kpR.current.slice(-50) : null,
                    }),
                });
            ...
            isCapturing.current = false;
        }, 400);  // 400ms for faster frame accumulation + smoother tracking
    };
    ...
    const stopAndAnalyze = async () => {
        ...
        try {
            if (framesR.current.length < 8) {
                clearTimeout(safetyTimer);
                setError('Not enough frames. Record for at least 8 seconds with full body visible.');
                setPhase('setup');
                return;
            }
            ...
            try {
                res = await fetch(ML_URL + '/live/finish', {
                    ...
                    exercise,
                    all_keypoints: kp,
                    user_height_cm: appData?.height_cm || null,
                    user_weight_kg: appData?.weight_kg || null,
                }),
            ...
            if (res.ok) {
                ...
                // Even if server returns error, try to show partial result
                // based on what was detected during recording
                let errMsg = 'Analysis failed. Please try again.';
                try {
                    const e = await res.json();
                    errMsg = e.detail || e.message || errMsg;
                } catch(_) {}
                // If we had good frames, show what we know
                if (kpR.current.length >= 4) {
                    const fallbackResult = {
                        overall_form:     'uncertain',
                        confidence:       0.3,
                        feedback:         'Analysis partially complete. ' + errMsg + ' Try again with full body visible.',
                        body_part_issues: [],
                        good_parts:       [],
                        total_reps:       0,
                    };
                ...
                <View style={s.overlay}>
                    ...
                    {phase==='recording' && (
                        <>
                            ...
                            <TouchableOpacity
                                style={[s.stopBtn, (goodFrames<10 || recordingSeconds<8) && s.stopBtnDis]}
                                onPress={stopAndAnalyze}
                                disabled={goodFrames<10 || recordingSeconds<8}>
                                <Text style={s.stopTxt}>
                                    {recordingSeconds < 8
                                        ? `⏱  Recording... ${8 - recordingSeconds}s more`
                                        : goodFrames < 10
                                        ? `🔵  Detecting... ${goodFrames}/10 frames`
                                        : '⏹  Stop & Get Results'}
                                </Text>
                            </TouchableOpacity>
                        </>
                        ...
                        <ScrollView style={s.panel} contentContainerStyle={{paddingBottom:30}}>
                            ...
                            {(result.overall_form === 'insufficient_data' || result.overall_form === 'uncertain') ? (
                                <>
                                    ...
                                    {/* Still show confidence % even on low confidence */}
                                    <View style={s.scoreRow}>
                                        <View style={[s.scoreBox, {borderColor:'#FFD700', flex:2}]}>
                                            <Text style={[s.scoreNum, {color:'#FFD700', fontSize:36}]}>
                                            ...
                                            <Text style={[s.scoreLbl, {marginTop:4, color:'#FFD700'}]}>
                                                📊 Record longer for higher confidence
                                            </Text>
                                        </View>
                                        <View style={s.scoreBox}>
                                            <Text style={{fontSize:32}}>⚠️</Text>
                                            <Text style={[s.scoreNum, {fontSize:12, color:'#FFD700', marginTop:4}]}>LOW</Text>
                                    ...
                                    {/* Show body part issues even on low confidence — rule-based, always valid */}
                                    {result.body_part_issues?.length > 0 && (
                                        <View style={s.section}>
                                            <Text style={s.sectionTitle}>⚠  Form Issues Detected</Text>
                                                ...
                                                <View key={i} style={s.issueRow}>
                                                    ...
                                                    <View style={{flex:1}}>
                                                        <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center'}}>
                                                            ...
                                                            {issue.mean_angle != null && (
                                                                <View style={s.angleTag}>
                                                                    <Text style={s.angleTxt}>{Math.round(issue.mean_angle)}°</Text>
                                                                </View>
                                                            )}
                                                        </View>
                                    ...
                                    {result.good_parts?.length > 0 && (
                                        <View style={s.section}>
                                            <Text style={[s.sectionTitle,{color:'#00FF88'}]}>✓  What Looks Good</Text>
                                        ...
                                        <View key={i} style={s.issueRow}>
                                            ...
                                            <View style={{flex:1}}>
                                                <View style={{flexDirection:'row', justifyContent:'space-between', alignItems:'center'}}>
                                                    ...
                                                    {issue.mean_angle != null && (
                                                        <View style={s.angleTag}>
                                                            <Text style={s.angleTxt}>
                                                                {Math.round(issue.mean_angle)}° 
                                                                {issue.expected_range ? ` (target: ${issue.expected_range[0]}-${issue.expected_range[1]}°)` : ''}
                                                            </Text>
                                                        </View>
                                                    )}
                                                </View>
...
const s = StyleSheet.create({
    ...
    angleTag: { backgroundColor:'rgba(255,150,0,0.2)', borderRadius:6, paddingHorizontal:6, paddingVertical:2, borderWidth:1, borderColor:'rgba(255,150,0,0.4)' },
    angleTxt: { color:'#FFA500', fontSize:10, fontWeight:'600' },
