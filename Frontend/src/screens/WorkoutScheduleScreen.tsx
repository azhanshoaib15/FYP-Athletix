    ...
    const logCaloriesToProgress = async (item: DaySchedule) => {
        ...
        try {
            if (token) {
                ...
                const durationMin = tickedCount * 12;

                // 1. Create a workout session (so Recent Workouts + counter populate)
                let sessionId: number | null = null;
                try {
                    const sessRes = await fetch(`${API_URL}/api/v1/workouts/sessions/start`, {
                        method: 'POST', headers: h,
                        body: JSON.stringify({
                            plan_id: null,
                            notes: item.day + ' — ' + item.focus,
                        }),
                    });
                    if (sessRes.ok) {
                        const sessData = await sessRes.json();
                        sessionId = sessData.id || null;
                        // End the session immediately with stats
                        if (sessionId) {
                            await fetch(`${API_URL}/api/v1/workouts/sessions/${sessionId}/end`, {
                                method: 'PATCH', headers: h,
                                body: JSON.stringify({
                                    total_calories_burned: totalCal,
                                    duration_minutes: durationMin,
                                    notes: tickedCount + ' exercises completed',
                                }),
                            }).catch(() => {});
                        }
                    }
                ...
                // 2. Also log to progress (for streak, XP, weight tracking)
                await fetch(`${API_URL}/api/v1/progress/`, {
                    method: 'POST', headers: h,
                    body: JSON.stringify({
                        total_calories_burned: totalCal,
                        workouts_completed: 1,
                        total_workout_minutes: durationMin,
                ...
                refreshSilent();
                // Second refresh after delay to ensure backend has committed
                setTimeout(() => refreshSilent(), 2000);
