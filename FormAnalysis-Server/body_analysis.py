...
GOOD_FORM_RANGES = {
    "Squats": {
        "right_knee":    (70,  110, 1.0),   # deep enough squat
        "left_knee":     (70,  110, 1.0),
        "right_hip":     (55,  100, 1.0),   # hip hinge
        "left_hip":      (55,  100, 1.0),
        "right_trunk":   (150, 180, 0.8),   # upright torso
        "left_trunk":    (150, 180, 0.8),
    },
    "PushUp": {
        "right_elbow":   (60,  100, 1.0),   # elbow at chest
        "left_elbow":    (60,  100, 1.0),
        "right_trunk":   (155, 180, 1.0),   # straight body line
        "left_trunk":    (155, 180, 1.0),
        "right_hip":     (150, 180, 0.8),   # hips not sagging
    },
    "Bicep curl": {
        "right_elbow":   (40,  80,  1.0),   # full range at bottom
        "right_shoulder_arm": (15, 50, 0.7), # no swinging
        "left_shoulder_arm":  (15, 50, 0.7),
    },
    "Shoulder press": {
        "right_elbow":   (150, 175, 1.0),   # full extension
        "left_elbow":    (150, 175, 1.0),
        "right_trunk":   (160, 180, 0.8),   # no arching back
        "left_trunk":    (160, 180, 0.8),
    },
    "Lateral Raises": {
        "right_shoulder_arm": (65, 95, 1.0), # raised to shoulder height
        "left_shoulder_arm":  (65, 95, 1.0),
        "right_elbow":        (150, 180, 0.6), # arms not overly bent
    },
    "Bench Press": {
        "right_elbow":   (60,  90,  1.0),   # bar to chest
        "left_elbow":    (60,  90,  1.0),
        "right_shoulder_arm": (35, 65, 0.7), # elbows not flaring
        "left_shoulder_arm":  (35, 65, 0.7),
    },
    "Incline beanch Press": {
        "right_elbow":   (65,  95,  1.0),
        "left_elbow":    (65,  95,  1.0),
        "right_shoulder_arm": (40, 70, 0.7),
        "left_shoulder_arm":  (40, 70, 0.7),
    },
    "Lat Pulldown": {
        "right_elbow":   (55,  85,  1.0),   # bar pulled to chest
        "right_trunk":   (155, 180, 0.7),   # slight lean back only
    },
    "BackRows": {
        "right_elbow":   (60,  95,  1.0),
        "right_trunk":   (145, 175, 0.8),   # controlled torso
    },
    "Lunges": {
        "right_knee":    (75,  100, 1.0),
        "left_knee":     (75,  100, 1.0),
        "right_trunk":   (155, 180, 0.8),
    },
    "Leg Press": {
        "right_knee":    (65,  95,  1.0),
        "left_knee":     (65,  95,  1.0),
    },
    "Tricep pushdown": {
        "right_elbow":   (30,  55,  1.0),   # full extension
        "right_shoulder_arm": (10, 35, 0.7), # elbows locked to sides
    },
    "Tricep Dips": {
        "right_elbow":   (65,  90,  1.0),
        "right_trunk":   (155, 180, 0.7),
    },
    "Pull Ups": {
        "right_elbow":   (50,  80,  1.0),
        "right_shoulder_arm": (30, 70, 0.8),
    },
    "Chest Fly": {
        "right_shoulder_arm": (20, 50, 1.0),
        "left_shoulder_arm":  (20, 50, 1.0),
        "right_elbow":        (150, 175, 0.6),
    },
    "Leg Raises": {
        "right_hip":     (15,  45,  1.0),
    ...
    "Plank": {
        "right_trunk":   (165, 180, 1.0),
        "left_trunk":    (165, 180, 1.0),
        "right_hip":     (165, 180, 1.0),
    },
}
...
def analyze_body_parts(seq: np.ndarray, exercise: str) -> dict:
    """
    ...
    ranges = GOOD_FORM_RANGES.get(exercise, {})
    if not ranges:
        return {"issues": [], "good_parts": [], "rep_count": 0,
    ...
    for angle_name, (min_good, max_good, weight) in ranges.items():
        ...
        mean_angle = float(angle_vals.mean())

        body_part = ANGLE_TO_BODY_PART.get(angle_name, angle_name)

        if mean_angle < min_good:
            deviation = min_good - mean_angle
            issue_type = "too_low"
        elif mean_angle > max_good:
            deviation = mean_angle - max_good
            issue_type = "too_high"
        else:
            # Within good range
